...
def blocks_blk_by_hash(block_hash=[], block_height=[]):
    ...
    for count, value in enumerate(block_hash):
        url = 'https://blockchain.info/rawblock/' + value + '?format=hex'
        data = requests.get(url).text
        with open(dir + "/block_" + str(block_height[count]) +".blk", "w") as blk_file:
            blk_file.write(data)
